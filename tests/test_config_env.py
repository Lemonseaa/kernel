"""Environment configuration tests."""

from __future__ import annotations

import os
import tempfile
import unittest
from pathlib import Path
from unittest.mock import patch

from checkpoint_ai.config import CheckpointAIConfig


def _without_checkpoint_ai_env() -> dict[str, str]:
    """Return process env without CheckpointAI-specific overrides."""

    return {key: value for key, value in os.environ.items() if not key.startswith("CHECKPOINT_AI_")}


class EnvConfigTest(unittest.TestCase):
    """Validate zero-code configuration loading."""

    def test_checkpoint_ai_config_loads_values_from_env_file(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            env_path = Path(tmp) / ".env"
            env_path.write_text(
                "\n".join(
                    [
                        "CHECKPOINT_AI_DEFAULT_PROVIDER=openai",
                        "CHECKPOINT_AI_OPENAI_API_KEY=test-openai",
                        "CHECKPOINT_AI_OPENAI_MODEL=gpt-test",
                        "CHECKPOINT_AI_DB_PATH=/data/checkpoint_ai.db",
                        "CHECKPOINT_AI_MAX_CONCURRENCY=4",
                        "CHECKPOINT_AI_LLM_CACHE_ENABLED=false",
                        "CHECKPOINT_AI_LLM_CACHE_TTL_SECONDS=42",
                    ]
                ),
                encoding="utf-8",
            )

            with patch.dict(os.environ, _without_checkpoint_ai_env(), clear=True):
                config = CheckpointAIConfig.from_env(env_path=env_path)

        self.assertEqual(config.default_provider, "openai")
        self.assertEqual(config.providers["openai"]["api_key"], "test-openai")
        self.assertEqual(config.providers["openai"]["model"], "gpt-test")
        self.assertEqual(config.sqlite_path, "/data/checkpoint_ai.db")
        self.assertEqual(config.max_concurrency, 4)
        self.assertFalse(config.llm_cache_enabled)
        self.assertEqual(config.llm_cache_ttl_seconds, 42.0)

    def test_environment_variables_override_env_file(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            env_path = Path(tmp) / ".env"
            env_path.write_text("CHECKPOINT_AI_DEFAULT_PROVIDER=minimax\n", encoding="utf-8")

            with patch.dict(os.environ, {"CHECKPOINT_AI_DEFAULT_PROVIDER": "openai"}, clear=False):
                config = CheckpointAIConfig.from_env(env_path=env_path)

        self.assertEqual(config.default_provider, "openai")


if __name__ == "__main__":
    unittest.main()
