...
class AuthWebhookCliTest(unittest.TestCase):
    ...
    def test_kernel_configure_webhooks_sends_supported_events(self) -> None:
        attempts: list[dict[str, object]] = []
        ...
        kernel = Kernel()
        sender = kernel.configure_webhooks(
            ["https://example.test/webhook"],
            transport=transport,
        )

        kernel.event_bus.emit(EventType.RUN_COMPLETED, {"run_id": "run-1", "state": "succeeded"})

        self.assertIs(kernel.webhook_sender, sender)
    ...
    def test_webhook_receiver_can_trigger_workflow(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(kernel.store.list_runs()[0]["user_request"], "external workflow")
    ...
    def test_cli_status_run_list_business_line_list_and_health(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            run = kernel.create_run("cli run")
            kernel.create_business_line("cli business")
            kernel.store.save_run(run)

            commands = [
                ["./kernel-cli", "--db", str(db_path), "status"],
                ["./kernel-cli", "--db", str(db_path), "run", "list"],
                ["./kernel-cli", "--db", str(db_path), "bl", "list"],
                ["./kernel-cli", "--db", str(db_path), "health"],
            ]
            outputs: list[str] = []
