...
from loop_harness.dryrun import DryRunProvider
from loop_harness.llm import LLMProvider, MiniMaxProvider, OpenAIProvider


@dataclass(slots=True)
class LoopHarnessConfig:
    """Configuration for loop_harness services."""

    providers: dict[str, dict[str, Any]] = field(default_factory=dict)
    default_provider: str = "minimax"
    dry_run: bool = False
    sqlite_path: str = "loop_harness.db"
    ...
    @classmethod
    def from_env(cls, env_path: str | Path = ".env") -> LoopHarnessConfig:
        """Build config from a .env file and process environment."""

        values = _load_env_file(Path(env_path))
        values.update(os.environ)
        default_provider = values.get("LOOP_HARNESS_DEFAULT_PROVIDER", "minimax").lower()
        ...
        return cls(
            ...
            dry_run=_bool_value(values.get("LOOP_HARNESS_DRY_RUN"), default=False),
            sqlite_path=values.get("LOOP_HARNESS_DB_PATH", "loop_harness.db"),
            max_concurrency=_int_value(values.get("LOOP_HARNESS_MAX_CONCURRENCY"), default=1),
            llm_cache_enabled=_bool_value(values.get("LOOP_HARNESS_LLM_CACHE_ENABLED"), default=True),
            llm_cache_max_size=_int_value(values.get("LOOP_HARNESS_LLM_CACHE_MAX_SIZE"), default=128),
            llm_cache_ttl_seconds=_float_value(
                values.get("LOOP_HARNESS_LLM_CACHE_TTL_SECONDS"),
                default=300.0,
            ),
            slow_task_threshold_seconds=_float_value(
                values.get("LOOP_HARNESS_SLOW_TASK_THRESHOLD_SECONDS"),
                default=5.0,
            ),
        )
...
def _provider_config(values: dict[str, str], provider: str) -> dict[str, Any]:
    ...
    prefix = f"LOOP_HARNESS_{provider}_"
    mapping = {
        "API_KEY": "api_key",
        "MODEL": "model",
        "BASE_URL": "base_url",
