...
class HealthChecker:
    ...
    def __init__(self, loop_harness: object | None = None) -> None:
        """Create a health checker for one loop_harness or standalone defaults."""

        self._tmp_dir: tempfile.TemporaryDirectory[str] | None = None
        if loop_harness is None:
            self._tmp_dir = tempfile.TemporaryDirectory()
            from loop_harness.loop_harness import LoopHarness

            loop_harness = LoopHarness(sqlite_path=Path(self._tmp_dir.name) / "loop_harness-health.db")
    ...
    def _check_provider(self) -> CheckResult:
        ...
        provider = getattr(self.loop_harness, "llm_provider", None) or MiniMaxProvider()
        try:
    ...
    def _check_event_bus(self) -> CheckResult:
        ...
        bus = getattr(self.loop_harness, "event_bus", None)
        if bus is None:
    ...
    def _check_approval_queue(self) -> CheckResult:
        ...
        gate = getattr(self.loop_harness, "human_gate", None)
    ...
    def _check_cost_budget(self) -> CheckResult:
        ...
        tracker = getattr(self.loop_harness, "cost_tracker", None)
        if tracker is None:
    ...
    def _check_recent_runs(self) -> CheckResult:
        ...
        store = getattr(self.loop_harness, "store", None)
        if store is None:
    ...
    def _store(self) -> SQLiteStore:
        """Return the loop_harness store or a temporary standalone store."""

        store = getattr(self.loop_harness, "store", None)
        if isinstance(store, SQLiteStore):
            return store
        self._tmp_dir = tempfile.TemporaryDirectory()
        return SQLiteStore(Path(self._tmp_dir.name) / "loop_harness-health.db")
