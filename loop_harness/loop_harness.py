...
New code should prefer `loop_harness.EvidenceHarness` for external workflow
...
from loop_harness.businessline import (
    BusinessLine,
    BusinessLineConfig,
...
from loop_harness.control.defaults import builtin_policies
from loop_harness.diagnostics import HealthChecker
from loop_harness.dryrun import DryRunContext, DryRunProvider
from loop_harness.evaluation import EvaluationGate, EvaluationRunner
from loop_harness.events import AuditLogger, Event, EventBus, EventType
from loop_harness.experiment import ExperimentLedger
from loop_harness.llm import ProviderRegistry
from loop_harness.memory import ContextManager, PersistentMemory, WorkingMemory
from loop_harness.models import Run, RunState, Task, TaskSpec, TaskState
from loop_harness.notification import NotificationManager
from loop_harness.observability import CostTracker, MetricsCollector, PerformanceMonitor
from loop_harness.persistence import SQLiteStore
from loop_harness.policy import ScenarioPolicy
from loop_harness.runtime import AgentRegistry, SimpleAgent
from loop_harness.tools import EchoTool, FileWriteTool, ToolPermission, ToolRegistry
from loop_harness.webhook import WebhookSender
...
class LoopHarness:
    ...
    def __init__(self, sqlite_path: str | Path | None = None, config: LoopHarnessConfig | None = None) -> None:
        """Create loop_harness services and wire their dependencies."""

        self.config = config or LoopHarnessConfig()
        ...
        self.health_checker = HealthChecker(loop_harness=self)
    ...
    @classmethod
    def from_env(cls, env_path: str | Path = ".env") -> LoopHarness:
        """Create a loop_harness from .env and environment variables."""

        config = LoopHarnessConfig.from_env(env_path)
    ...
    def configure_webhooks(
        ...
        """Configure outbound webhooks for supported loop_harness events."""
    ...
    def run(self, run: Run | str, tasks: list[TaskSpec] | None = None) -> Run | Coroutine[Any, Any, Run]:
        ...
        if isinstance(run, Run):
            return self._execute_run(run)
        if tasks is None:
            raise ValueError("LoopHarness.run(description, tasks) requires tasks.")
    ...
    def on(
        ...
        """Subscribe to loop_harness events."""
