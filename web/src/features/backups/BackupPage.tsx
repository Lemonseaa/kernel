...
export function BackupPage() {
  ...
  return (
    <>
      <PageHeader title="Backup" description="Create and restore SQLite backups with explicit operator action." />
      <div className="mb-5">
        ...
        {backups.data?.length ? (
          <DataTable<BackupRecord>
            ...
            columns={[
              ...
              { key: "path", header: "Path", render: (row) => <span className="text-xs text-muted">{row.path}</span> },
              {
                key: "action",
                header: "Action",
                render: (row) => (
                  <button
                    ...
                    onClick={() => {
                      if (window.confirm(`Restore backup ${row.label}? This replaces current persisted state.`)) {
