...
from checkpoint_ai.dryrun import DryRunProvider
from checkpoint_ai.llm import LLMProvider, MiniMaxProvider, OpenAIProvider


@dataclass(slots=True)
class CheckpointAIConfig:
    """Configuration for checkpoint_ai services."""

    providers: dict[str, dict[str, Any]] = field(default_factory=dict)
    default_provider: str = "minimax"
    dry_run: bool = False
    sqlite_path: str = "checkpoint_ai.db"
    ...
    instance_id: str = field(default_factory=lambda: f"checkpoint_ai-{uuid4()}")
    ha_lease_ttl_seconds: float = 30.0
    ha_heartbeat_seconds: float = 10.0

    @classmethod
    def from_env(cls, env_path: str | Path = ".env") -> CheckpointAIConfig:
        """Build config from a .env file and process environment."""

        values = _load_env_file(Path(env_path))
        values.update(os.environ)
        default_provider = values.get("CHECKPOINT_AI_DEFAULT_PROVIDER", "minimax").lower()
        ...
        return cls(
            ...
            dry_run=_bool_value(values.get("CHECKPOINT_AI_DRY_RUN"), default=False),
            sqlite_path=values.get("CHECKPOINT_AI_DB_PATH", "checkpoint_ai.db"),
            max_concurrency=_int_value(values.get("CHECKPOINT_AI_MAX_CONCURRENCY"), default=1),
            llm_cache_enabled=_bool_value(values.get("CHECKPOINT_AI_LLM_CACHE_ENABLED"), default=True),
            llm_cache_max_size=_int_value(values.get("CHECKPOINT_AI_LLM_CACHE_MAX_SIZE"), default=128),
            llm_cache_ttl_seconds=_float_value(
                values.get("CHECKPOINT_AI_LLM_CACHE_TTL_SECONDS"),
                default=300.0,
            ),
            slow_task_threshold_seconds=_float_value(
                values.get("CHECKPOINT_AI_SLOW_TASK_THRESHOLD_SECONDS"),
                default=5.0,
            ),
            ha_enabled=_bool_value(values.get("CHECKPOINT_AI_HA_ENABLED"), default=False),
            instance_id=values.get("CHECKPOINT_AI_INSTANCE_ID", f"checkpoint_ai-{uuid4()}"),
            ha_lease_ttl_seconds=_float_value(
                values.get("CHECKPOINT_AI_HA_LEASE_TTL_SECONDS"),
                default=30.0,
            ),
            ha_heartbeat_seconds=_float_value(
                values.get("CHECKPOINT_AI_HA_HEARTBEAT_SECONDS"),
                default=10.0,
            ),
        )
...
def _provider_config(values: dict[str, str], provider: str) -> dict[str, Any]:
    ...
    prefix = f"CHECKPOINT_AI_{provider}_"
    mapping = {
        "API_KEY": "api_key",
        "MODEL": "model",
        "BASE_URL": "base_url",
