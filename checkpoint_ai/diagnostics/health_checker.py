...
class HealthChecker:
    ...
    def __init__(self, checkpoint_ai: object | None = None) -> None:
        """Create a health checker for one checkpoint_ai or standalone defaults."""

        self._tmp_dir: tempfile.TemporaryDirectory[str] | None = None
        if checkpoint_ai is None:
            self._tmp_dir = tempfile.TemporaryDirectory()
            from checkpoint_ai.checkpoint_ai import CheckpointAI

            checkpoint_ai = CheckpointAI(sqlite_path=Path(self._tmp_dir.name) / "checkpoint_ai-health.db")
    ...
    def _check_provider(self) -> CheckResult:
        ...
        provider = getattr(self.checkpoint_ai, "llm_provider", None) or MiniMaxProvider()
        try:
    ...
    def _check_event_bus(self) -> CheckResult:
        ...
        bus = getattr(self.checkpoint_ai, "event_bus", None)
        if bus is None:
    ...
    def _check_approval_queue(self) -> CheckResult:
        ...
        gate = getattr(self.checkpoint_ai, "human_gate", None)
    ...
    def _check_cost_budget(self) -> CheckResult:
        ...
        tracker = getattr(self.checkpoint_ai, "cost_tracker", None)
        if tracker is None:
    ...
    def _check_recent_runs(self) -> CheckResult:
        ...
        store = getattr(self.checkpoint_ai, "store", None)
        if store is None:
    ...
    def _store(self) -> SQLiteStore:
        """Return the checkpoint_ai store or a temporary standalone store."""

        store = getattr(self.checkpoint_ai, "store", None)
        if isinstance(store, SQLiteStore):
            return store
        self._tmp_dir = tempfile.TemporaryDirectory()
        return SQLiteStore(Path(self._tmp_dir.name) / "checkpoint_ai-health.db")
