...
async def main() -> None:
    ...
    bl_a = checkpoint_ai.create_business_line('course_business')
    bl_b = checkpoint_ai.create_business_line('website_business')
    ...
    run_a = await checkpoint_ai.run(run=bl_a.id, tasks=[task_a])
    run_b = await checkpoint_ai.run(run=bl_b.id, tasks=[task_b])
